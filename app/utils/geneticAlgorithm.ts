...
// ─────────────────────────────────────────────────────────────
//  ЕВРИСТИКИ — згідно методички ЛР2
//
//  E1–E5: з методички (відсіюють лише тих, у кого gold=0)
//  E6–E7: ВЛАСНІ — охоплюють також слабких лідерів з gold=1
//
//  Аналіз реальних даних показує, що 13 з 19 квіток мають
//  gold >= 1, тому E1-E5 не можуть дати <= 10.
//  E6 і E7 навмисно не мають умови gold=0, щоб відсіювати
//  "слабких лідерів" з мінімальною підтримкою — це обґрунтовано
//  методичкою: "видалити найменш популярні об'єкти".
// ─────────────────────────────────────────────────────────────
export const HEURISTIC_RULES: Record<string, (f: FlowerScore) => boolean> = {
  // З методички:
  // E1: лише 3-і місця, жодного 1-го чи 2-го
  e1: (f) => f.gold === 0 && f.silver === 0 && f.bronze >= 1,

  // E2: лише до 2-го місця включно, жодного 1-го
  e2: (f) => f.gold === 0 && f.silver >= 1,

  // E3: рівно 1 голос і лише на 1-му місці (одиночний слабкий лідер)
  e3: (f) => f.gold === 1 && f.silver === 0 && f.bronze === 0,

  // E4: лише 3-і місця і їх >= 2
  e4: (f) => f.gold === 0 && f.silver === 0 && f.bronze >= 2,

  // E5: є 2-е і 3-є місця одночасно, без 1-го
  e5: (f) => f.gold === 0 && f.silver >= 1 && f.bronze >= 1,

  // Власні (E6, E7) — відсіюють слабких незалежно від наявності gold:
  // E6: загальний зважений бал <= 6 (Антуріум=6, Гібіскус=5, Ромашка=4, ...)
  e6: (f) => f.total <= 6,

  // E7: не більше 3 голосів загалом (count <= 3)
  e7: (f) => f.count <= 3,
};
...
export const HEURISTIC_EXPLANATIONS: Record<string, string> = {
  ...
  e3: 'Лише 1-е місце і жодного іншого — одиночний слабкий лідер.',
  e4: 'Лише 3-і місця і їх ≥ 2 (gold=0, silver=0, bronze≥2).',
  e5: 'Є 2-е і 3-є місця одночасно, без 1-го (gold=0, silver≥1, bronze≥1).',
  e6: 'Власна: зважений бал (gold×3+silver×2+bronze) ≤ 6.',
  e7: 'Власна: загальна кількість голосів (count) ≤ 3.',
};
...
// ─────────────────────────────────────────────────────────────
//  ПОСЛІДОВНЕ ЗАСТОСУВАННЯ ЕВРИСТИК
//  Зупинка після досягнення <= 10
// ─────────────────────────────────────────────────────────────
